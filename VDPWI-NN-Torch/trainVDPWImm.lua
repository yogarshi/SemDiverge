...
Training script for semantic relatedness prediction on the Twitter dataset.
  ...
  --loadm (default 'none') saved model to load
]]
...
print (data_dir .. 'vocab-cased.txt')
-- load vocab
...
if args.loadm ~= 'none' then
  include('./models/AlignMaxDropLeakyMulti.lua')
  include('./models/very_deep.lua')
  local modeladdr = args.loadm --data_dir .. 'saved_model/bestModel.ep5.th'
...
for i = 1, num_epochs do
  ...
  --end
    ...
    print('Finished testing in ' .. ( sys.clock() - start) )
    print('Best dev model was saved_model/bestModel.ep' .. best_dev_model .. ".th")
    local score = pearson(test_predictions, test_dataset.labels)
    --outfile = io.open(args.folder .. "predictions/test_preds_" .. i, "w")
    outfile = io.open(args.folder .. "predictions/test_preds_final", "w")
